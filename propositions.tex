...
\end{align}
$\dfrac{d}{dp} \dfrac{g(p)}{p}$ being an increasing function of $p$ (from \eqref{property_decreasing}), with $p_1'<p_1\le p_N<p_N'$, equation (\ref{bits_equal1}) implies $\gamma >\delta$. Hence, transmission time in the policy $Y$, $\left( s_{N+1}-s_1+\gamma-\delta\right)$, is greater than transmission time in policy $X$ i.e. $(s_{N+1}-s_1)$.
\end{proof}
\begin{figure}
\centering

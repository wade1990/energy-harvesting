...
If, in policy $Y$ transmission continues after $s_{i+1}$ i.e. $s_{N'+1}'>s_{i+1}$, then the amount of energy used by $Y$ in interval $[s_{i},s_{i+1}]$ can be lower bounded by $p_i'(s_{i+1}-s_i)$, by Lemma \ref{lemma_increasing_power}. $p_i'(s_{i+1}-s_i)$ is more than $p_i(s_{i+1}-s_i)$, which is the energy used by policy $X$. But by structure \eqref{claim3}, $X$ uses all energy available by $s_{i+1}$. So $Y$ is not feasible with respect to the energy constraint. 

If $s_{N'+1}'\le s_{i+1}$, then it can be easily verified by (\ref{property_decreasing}) that $Y$ transmits strictly less number of bits in interval $[s_i,s_{N'+1}]$ than $X$ in interval $[s_{i},s_{i+1}]$. Both policies being same till $s_i$, we conclude that $Y$ transmits less than $B_0$  bits and thus it is not optimal.
